 ...
 * Name = CurrentPokemonRight
 * DateTime = Fri Nov 26 2021 13:51:16 GMT+0800 (中国标准时间)
 * Author = QAQGaeBolg
 * FileBasename = CurrentPokemonRight.ts
 * FileBasenameNoExtension = CurrentPokemonRight
 * URL = db://assets/script/components/CurrentPokemonRight.ts
 * ManualUrl = https://docs.cocos.com/creator/3.3/manual/en/
 *
 */
 
@ccclass('CurrentPokemonRight')
export class CurrentPokemonRight extends cc.Component {
    ...
    update (deltaTime: number) {
        if (FightSceneTwo.battleState == BattleState.LoadBattle_3) {
            this.node.setPosition(new cc.Vec3(160, 240))
            this.node.getComponent(cc.UITransform).width = 160
